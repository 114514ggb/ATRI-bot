...
class FuncCall:
    ...
    async def _init_mcp_clients(self) -> None:
        ...
        if not os.path.exists(mcp_json_file):
            ...
            print(f"未找到 MCP 服务配置文件，已创建默认配置文件 {mcp_json_file}")
            return
    ...
    async def _init_mcp_client_task_wrapper(
        ...
        try:
            ...
            await event.wait()
            print(f"收到 MCP 客户端 {name} 终止信号")
            await self._terminate_mcp_client(name)
        except Exception as e:
            import traceback

            traceback.print_exc()
            print(f"初始化 MCP 客户端 {name} 失败: {e}")
    ...
    async def _init_mcp_client(self, name: str, config: dict) -> None:
        ...
        try:
            ...
            print(f"已连接 MCP 服务 {name}, Tools: {tool_names}")
            return
        except Exception as e:
            import traceback
