...
class OneBotSendClient:
    ...
    async def async_send(self, action: str, params: dict, echo: bool = False) -> Optional[dict]:
        """通用的发送请求
    ...
    async def send_group_poke(self, group_id: int, user_id: int) -> None:
        """发送群戳一戳"""
        await self.async_send("group_poke", {"group_id": group_id, "user_id": user_id})

    async def send_group_json(self, group_id: int, json_dict: dict) -> None:
        """发送群 JSON 卡片消息"""
        await self.send_group_msg(group_id, [{"type": "json", "data": json_dict}])
    ...
    async def set_group_add_request(
        ...
        await self.async_send("set_group_add_request", payload)
    ...
    def __getattr__(self, item: str):
        ...
        async def _dynamic_api_call(echo: bool = False, **kwargs) -> dict | None:
