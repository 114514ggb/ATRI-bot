...
class TextChunker:
    ...
    def fixed_size_chunking(text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
        ...
        该方法将输入文本按照指定的块大小和重叠量进行分割，
        生成连续的文本块。每个块的大小为chunk_size,相邻块之间
        有chunk_overlap个字符的重叠。
    ...
    def sentence_chunking(text: str, max_chunk_sentences: int = 3) -> List[str]:
        ...
        该方法将输入文本按句子分割，然后按照指定的最大句子数量
        将连续的句子组合成文本块。每个块包含最多max_chunk_sentences个句子。
        缺点:
        句子长度差异大：有的句子很短，有的很长，导致 Chunk 大小不均匀，可能影响后续处理和检索稳定性。
        简单的基于标点的分割可能不准确（例如,Mr. Smith 中的 .）。需要更可靠的 NLP 工具。
        对于代码、列表、或者没有明确句子结构的文本（如 JSON, YAML)效果不佳。
        ...
        Args:
            text (str): 需要分块的输入文本
            max_chunk_sentences (int, optional): 每个文本块的最大句子数量，默认为3
            
        Returns:
            List[str]: 分割后的文本块列表，每个块由多个句子组成
        """
...
class RecursiveCharacterTextSplitter:
    ...
    def _split_text_recursive(self, text:str, separators):
        """
        递归分割文本
        ...
        for i, split in enumerate(splits):
            ...
            if split_len > self.chunk_size:
                # 先处理暂存集合
                if temp_chunks:
                    merged = self._merge_chunks(temp_chunks, separator)
            ...
            # 如果加入当前片段后会超过chunk_size
            elif temp_length + split_len + (len(separator) if temp_chunks else 0) > self.chunk_size:
                # 处理暂存集合
                if temp_chunks:
                    merged = self._merge_chunks(temp_chunks, separator)
                    final_chunks.extend(merged)
                ...
                # 将当前片段加入新的暂存集合
                temp_chunks.append(split)
                temp_length += split_len
            
            else:
                # 正常加入暂存集合
                temp_chunks.append(split)
                temp_length += split_len + (len(separator) if len(temp_chunks) > 1 else 0)
    ...
    def _merge_chunks(self, chunks, separator:str):
        """
        合并小块为符合大小的块
        ...
        for chunk in chunks:
            ...
            if current_len + chunk_len + sep_len <= self.chunk_size:
                current.append(chunk)
                current_len += chunk_len + sep_len
            else:
                if current:
    ...
    def _add_overlap(self, chunks):
        ...
        if list_len > 2:
            ...
            for i in range(list_len-1):
                overlapped_chunks.append(tail + chunks[i] + chunks[i+1][:chunk_overlap])
                tail = chunks[i][-chunk_overlap:]
        else:
            overlapped_chunks.append(chunks[0] + chunks[1][:chunk_overlap])
            tail = chunks[0][-chunk_overlap:]
        
        overlapped_chunks.append(tail + chunks[-1])
