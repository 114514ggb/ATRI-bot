...
tool_json = {
    "name": "memory_search",
    "description": "基于语义的记忆向量检索工具。当需要回忆过去发生的事件、了解某人的特定信息、或者查询知识库中的设定时使用。支持按时间范围、用户、群组或特定范围进行筛选",
    "properties": {
        "question_text": {
            "type": "string",
            "description": "你要查询内容的相关文本。例如：'张三是'、'喜欢什么电影'就能返回相关的信息,只能查询语义相近的。如果不填则按时间倒序返回最近的记忆。",
        },
        "search_scope": {
            "type": "string",
            "enum": [
