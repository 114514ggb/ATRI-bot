...
class AIContextCommands:
    ...
    def _register_command(self):
        ...
        if self.user_global_context:
            ...
            async def ai_context_handler(message_data:MessageEventEnvelope, action: str, target: str = None, detail: bool = False):
                ...
                if action == "role":
                    await self._handle_set_role_user(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "current":
                    await self._handle_current_role_user_(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "list":
                    await self._handle_list_roles(send_client, group_id, detail)
                elif action == "reload":
                    await self._handle_reload_roles(send_client, group_id, user_id=message_data.user_id)
                elif action == "reset":
                    await self._handle_reset_context_user_(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "info":
                    await self._handle_context_info_user(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "user":
                    await self._handle_get_user_info(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "active":
                    await self._handle_group_active_chat(send_client, group_id, target, user_id=message_data.user_id)
        else:
            @self.command_system.register_command(
                name="chat",
                description="AI上下文和角色管理命令",
            ...
            async def ai_context_handler(message_data:MessageEventEnvelope, action: str, target: str = None, detail: bool = False):
                ...
                if action == "role":
                    await self._handle_set_role(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "current":
                    await self._handle_current_role(send_client, group_id)
                elif action == "list":
                    await self._handle_list_roles(send_client, group_id, detail)
                elif action == "reload":
                    await self._handle_reload_roles(send_client, group_id, user_id=message_data.user_id)
                elif action == "reset":
                    await self._handle_reset_context(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "info":
                    await self._handle_context_info(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "user":
                    await self._handle_get_user_info(send_client, group_id, target, user_id=message_data.user_id)
                elif action == "active":
                    await self._handle_group_active_chat(send_client, group_id, target, user_id=message_data.user_id)
    ...
    async def _handle_set_role(self, send_client: SendClientBase, group_id: str, role_name: str, user_id:int):
        """处理角色切换"""
        if not role_name:
    ...
    async def _handle_set_role_user(self, send_client: SendClientBase, group_id: str, role_name: str, user_id:int):
        """处理角色切换,全局上下文版本"""
        if not role_name:
            await send_client.send_group_msg(
                group_id, 
    ...
    async def _handle_current_role(self, send_client: SendClientBase, group_id: int):
        """处理查看当前角色"""
        ...
        await send_client.send_group_msg(group_id, message)

    async def _handle_current_role_user_(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """处理查看当前角色_user"""
        if target:
            self.permissions_management.has_permission(user_id, 2)
            try:
    ...
    async def _handle_list_roles(self, send_client: SendClientBase, group_id: str, detail: bool = False):
        """处理列出角色"""
        ...
        await send_client.send_group_merge_text(
            group_id, 
            message,
            source = "查看角色列表"
        )
    
    async def _handle_reload_roles(self, send_client: SendClientBase, group_id: str, user_id:int):
        """处理重载角色配置"""
        ...
        try:
            ...
            await send_client.send_group_msg(group_id, error_message)
            self.log.error(f"角色配置重载失败：{e}")
    
    async def _handle_reset_context(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """处理重置上下文"""
    ...
    async def _handle_reset_context_user_(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """处理重置上下文,user版本"""
    ...
    async def _handle_context_info(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """处理查看上下文信息"""
    ...
    async def _handle_context_info_user(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """处理查看上下文信息,全局上下文版本"""
        current_group_id = group_id
        if target:
            self.permissions_management.has_permission(user_id, 2)
        ...
        message += f"预计上下文token: {context.count_estimate_tokens()}\n"
        message += f"上次响应token: {context.total_tokens}\n"
    ...
    async def _handle_get_user_info(self, send_client: SendClientBase, group_id: str, target:str, user_id:int):
        """获取维护的user_info文档"""
    ...
    async def _handle_group_active_chat(self, send_client: SendClientBase, group_id: int, target:str, user_id:int):
        """切换群聊主动聊天参数"""
        self.permissions_management.has_permission(user_id, 2)
